...
const TIMED_OUT_WAITING_FOR_PARENT_REPLY_RE =
  /Timed out waiting for parent reply/;
...
describe("ParentBridge", () => {
  it("resolves ask_parent requests when the parent replies", async () => {
    ...
    const replyPromise = bridge.askParent("agent-1", "Need approval", {
      sessionId: SESSION_A,
    });
  ...
  it("drains queued bridge traffic by parent session", async () => {
    ...
    const replyPromise = bridge.askParent("agent-2", "Need sign-off", {
      sessionId: SESSION_B,
    });
    ...
    expect(
      queuedForA.map(({ agentId, kind, message, sessionId }) => ({
        agentId,
        kind,
        message,
        sessionId,
    ...
    expect(
      queuedForB.map(({ agentId, kind, message, sessionId }) => ({
        agentId,
        kind,
        message,
        sessionId,
  ...
  it("does not enqueue asks for already-aborted signals", async () => {
    ...
    await expect(
      bridge.askParent("agent-3", "Still there?", {
        sessionId: SESSION_A,
        signal: controller.signal,
      })
    ).rejects.toMatchObject({
      name: "AbortError",
      message: "ask_parent aborted for agent agent-3",
    });
  ...
  it("removes stale queued asks when they time out", async () => {
    ...
    await expect(replyPromise).rejects.toThrow(
      TIMED_OUT_WAITING_FOR_PARENT_REPLY_RE
    );
    expect(bridge.drainAllMessagesForSession(SESSION_A)).toEqual([]);
  ...
  it("only resolves replies from the matching parent session", async () => {
    ...
    const replyPromise = bridge.askParent("agent-4", "Need approval", {
      sessionId: SESSION_A,
    });
    const [queued] = bridge.drainMessages("agent-4");

    expect(
      bridge.replyToAsk(queued.requestId, "Wrong session", {
        sessionId: SESSION_B,
      })
    ).toBe(false);
    expect(
      bridge.replyToAsk(queued.requestId, "Approved", { sessionId: SESSION_A })
    ).toBe(true);
  ...
  it("disposes queued state for only the matching parent session", async () => {
    ...
    const sessionAReply = bridge.askParent("agent-a", "Need session A", {
      sessionId: SESSION_A,
    });
    const sessionBReply = bridge.askParent("agent-b", "Need session B", {
      sessionId: SESSION_B,
    });
    const [queuedA] = bridge.drainMessages("agent-a");

    const deliveredSessionA = bridge.messageParent(
      "agent-a",
      "Delivered session A update",
      { sessionId: SESSION_A }
    );
    bridge.drainMessages("agent-a");
    bridge.messageParent("agent-a", "Session A update", {
      sessionId: SESSION_A,
    });
    bridge.messageParent("agent-b", "Session B update", {
      sessionId: SESSION_B,
    });
    ...
    await expect(sessionAReply).rejects.toMatchObject({
      name: "AbortError",
      message: "Session A closed",
    });
    expect(
      bridge.getMessage(deliveredSessionA.requestId, { sessionId: SESSION_A })
    ).toBeUndefined();
    expect(
      bridge.replyToAsk(queuedA.requestId, "Late", { sessionId: SESSION_A })
    ).toBe(false);
    expect(bridge.drainAllMessagesForSession(SESSION_A)).toEqual([]);
    expect(bridge.drainAllMessagesForSession(SESSION_B)).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          agentId: "agent-b",
          kind: "ask",
          message: "Need session B",
          sessionId: SESSION_B,
        }),
        expect.objectContaining({
          agentId: "agent-b",
          kind: "message",
          message: "Session B update",
          sessionId: SESSION_B,
        }),
      ])
    );
    ...
    await expect(sessionBReply).rejects.toMatchObject({
      name: "AbortError",
      message: "cleanup",
    });
  });
  ...
  it("disposes queued state and rejects pending asks for an agent", async () => {
    ...
    const replyPromise = bridge.askParent("agent-2", "Need input", {
      sessionId: SESSION_A,
    });
    ...
    bridge.messageParent("agent-2", "leftover message", {
      sessionId: SESSION_A,
    });
