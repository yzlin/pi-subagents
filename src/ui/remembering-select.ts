...
export interface RememberingSelectOption {
  value: string;
  label: string;
  description?: string;
}

interface RememberingSelectConfig {
...
function getAliasSelectionMove(
  data: string,
  pageSize: number
...
class RememberingSelectComponent implements Component {
  ...
  constructor(
    ...
    this.list.onSelectionChange = (item) => {
      const nextIndex = this.options.findIndex(
  ...
  render(width: number): string[] {
    ...
    const row = (text = "") =>
      border("│") +
      truncateToWidth(padVisibleText(text, innerWidth), innerWidth) +
      border("│");
    ...
    const headerRows = this.infoLines.map((line) =>
      row(this.theme.fg("dim", line))
    );
...
export function showRememberingSelect(
  ...
  return ctx.ui.custom<string | undefined>(
    (_tui, theme, _keybindings, done) =>
      new RememberingSelectComponent(
        title,
        config.infoLines ?? [],
        theme,
        options,
